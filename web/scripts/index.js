...
function submitForm(event) {
  ...
  let xmlCuentas = []
	arrayCuentas.forEach((_cuenta) => {
		const nombre = (_cuenta.debe !== '$NaN')? _cuenta.cuenta : '    '+_cuenta.cuenta

		xmlCuentas.push(`<Cuentas_Detalles>${nombre}</Cuentas_Detalles>`)
		
		_cuenta.subcuentas.forEach(_subcuenta => {
			xmlCuentas.push((_cuenta.debe !== '$NaN')?
								`<Cuentas_Detalles>  ${_subcuenta.subcuenta}</Cuentas_Detalles>` :
								`<Cuentas_Detalles>      ${_subcuenta.subcuenta}</Cuentas_Detalles>`)
		})
	})
	...
	let xmlParcial = []
  arrayCuentas.forEach((_cuenta) => {
		xmlParcial.push(`<Parcial></Parcial>`)
		
		
		_cuenta.subcuentas.forEach(_subcuenta => {
			xmlParcial.push(`<Parcial>${_subcuenta.parcial}</Parcial>`)
		})
	})
	...
	arrayCuentas.forEach((_cuenta) => {
			if (_cuenta.debe === '$NaN') {
				xmlDebe.push(`<Debe></Debe>`)
			}
			else {
				xmlDebe.push(`<Debe>${_cuenta.debe}</Debe>`)
			}
  ...
  let xmlHaber = []
	arrayCuentas.forEach((_cuenta) => {
		if (_cuenta.haber === '$NaN') {
			xmlHaber.push(`<Haber></Haber>`)
		}
		else {
			xmlHaber.push(`<Haber>${_cuenta.haber}</Haber>`)
		}
	...
	let xmlBody = ''
	for(let i=0; i < xmlCuentas.length+1; i++){
		const xmlFecha = (i === 0)? `<Fecha>${fecha}</Fecha>` : `<Fecha></Fecha>`
		const primeros = xmlFecha + xmlCuentas[i] + xmlParcial[i] + xmlDebe[i] + xmlHaber[i]
		const ultimo = xmlFecha + `<Cuentas_Detalles>${detalle}</Cuentas_Detalles><Parcial></Parcial><Debe></Debe><Haber></Haber>`

		if (i !== xmlCuentas.length) xmlBody += `<Registro>${primeros}</Registro>`
		else xmlBody += `<Registro>${ultimo}</Registro>`
	}
